<#
 # Replace words in multiple documents
 # Usage:

 .\FindAndReplaceInMultiDocuments -pathToWordDocuments "C:\.....\myDirectory\" `
                                  -wordToSearchFor "Search This" `
                                  -wordToReplaceWith "Replace With"
#>
...
Param(
    ...
    [parameter(Mandatory=$true)]
    [string]
    $wordToSearchFor = [ref]"Search This",

    [parameter(Mandatory=$true)]
    [string]
    $wordToReplaceWith = [ref]"Replace With"
)
...
$files = Get-ChildItem -Path $pathToWordDocuments -Filter *.doc?
